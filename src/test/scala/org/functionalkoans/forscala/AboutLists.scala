...
//   When map {...} is actually map({..}).
//   {...} allow multiple expressions.
//   Parenthesis can be used to make subexpressions. Curly braces can be used to make blocks of code.
//   Scala infers the braces when a parameter is a function, e.g. in list.map(_ * 2) the braces 
//   are inferred, it's just a shorter form of list.map({_ * 2}). 
// ================================================================================================
...
class AboutLists extends KoanSuite {
  ...
  koan("Lists can be accessed by position") {
  ...
  koan("Lists can be 'reduced' with a mathematical operation") {
    ...
    a.reduceLeft((x:Int,y:Int) => x + y) should equal(16)
